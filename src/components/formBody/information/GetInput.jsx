...
const GetInput = (props) => {
  const [chosen_loaiGiayTo, setChosen_loaiGiayTo] =
    useState(initialLoaiGiayToID);
  ...
  const setInitialData = (id) => {
    ...
    setChecked_list(
      data.find((x) => x.id === id)
        ? data.find((x) => x.id === id).fields.map((item) => item)
        : []
    );
  };
  useEffect(() => {
    data && data.find((x) => x.id === initialLoaiGiayToID)
  ...
  useEffect(() => {
    formik.resetForm();
    setChecked_list(
      data.find((x) => x.id === chosen_loaiGiayTo)
        ? data
  ...
  const handleSubmit = () => {
    ...
    if (
      formik.values.loaiGiayTo === 1 &&
      Object.keys(formik.values.frontImage).length <= 0
    ) {
      alert("Điền đầy đủ các mục bắt buộc");
      return;
    }
    if (
      (formik.values.loaiGiayTo === 3 &&
        Object.keys(formik.values.frontImage).length <= 0 &&
        Object.keys(formik.values.backImage).length <= 0) ||
      formik.values.backImage.file === null ||
      formik.values.frontImage.file === null
    ) {
      alert("Điền đầy đủ các mục bắt buộc");
      return;
    }
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              e.target.checked === true
                                ? setChecked_list([...checked_list, item])
                                : setChecked_list([
                                    ...checked_list.filter(
                                      (x) => x.field !== item.field
                                    ),
